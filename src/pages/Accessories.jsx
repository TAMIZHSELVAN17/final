...
//   const { interestedItems, addToInterested } = useInterested();

//   useEffect(() => {
//     axios
//       .get('https://daas-computers.onrender.com/api/product/all')
//       .then((res) => {
...
//         >
//           <div
//             className="bg-white w-full max-w-sm rounded-xl shadow-lg p-4 z-50 relative"
...
//                 const isAlreadyInterested = interestedItems.some((i) => i.name === item.name);
...
//                         isAlreadyInterested ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'
//                       }`}
//                       onClick={() => {
//                         if (!isAlreadyInterested) {
//                           addToInterested(item);
//                           toast.success('Added to Interested');
//                         } else {
//                           toast.info('Already in Interested List');
//                         }
//                       }}
//                       disabled={isAlreadyInterested}
//                     >
//                       {isAlreadyInterested ? 'Interested ✔' : 'Add to Interested'}
//                     </button>
//                   </div>
//                 );
//               })
...
import cartIcon from '../assets/cart1.png'; // ⬅️ Make sure this path matches your image
...
const Accessories = () => {
  ...
  return (
    <div className="min-h-screen bg-blue-100 p-4 sm:p-6 flex flex-col items-center relative">
      ...
      <div className="flex flex-col lg:flex-row w-full max-w-7xl gap-6">
        <main className="w-full lg:w-4/5">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {loading ? (
              <p className="col-span-full text-center text-blue-600 font-medium">Loading...</p>
            ) : filteredItems.length === 0 ? (
              <p className="col-span-full text-center text-gray-500 italic">No items found.</p>
            ) : (
              filteredItems.map((item, index) => {
                const isAlreadyInterested = interestedItems.some((i) => i.name === item.name);

                return (
                  <div key={index} className="bg-white p-3 rounded-xl shadow hover:shadow-md transition">
                    <img
                      src={item.image}
                      alt={item.name}
                      onError={(e) =>
                        (e.target.src = 'https://via.placeholder.com/300x200?text=Accessory')
                      }
                      className="w-full h-36 object-cover rounded mb-2"
                    />
